...
pub fn parse_cmd(cmd: &Value) -> anyhow::Result<ParsedCommand> {
    ...
    match cmd {
        ...
        Value::Mapping(m) => {
            ...
            let prog = prog
                .as_str()
                .ok_or(anyhow::anyhow!("Prog is not a string"))?;

            let args = m.get("args").unwrap_or(default_args);
            let args = args
                .as_sequence()
                .ok_or(anyhow::anyhow!("Args is not a sequence"))?;
            let args = args
                .iter()
                .map(|a| {
                    a.as_str()
                        .ok_or(anyhow::anyhow!("Arg {:?} is not a string", a))
                        .map(|s| s.to_string())
                })
            ...
            let cmd = m
                .get("cmd")
                .ok_or(anyhow::anyhow!("No cmd key in command"))?;
            let cmd = cmd.as_str().ok_or(anyhow::anyhow!("Cmd is not a string"))?;

            let pre_msg = m
                .get("pre_msg")
                .map(|v| v.as_str().ok_or(anyhow::anyhow!("pre_msg is not a string")))
                .transpose()?;
            let post_msg = m
                .get("post_msg")
                .map(|v| {
                    v.as_str()
                        .ok_or(anyhow::anyhow!("post_msg is not a string"))
                })
                .transpose()?;
            ...
            let pre_cmds = m
                ...
                .map(|v| {
                    v.as_sequence()
                        .ok_or(anyhow::anyhow!("pre_cmds is not a sequence"))
                })
                .transpose()?;
            let pre_cmds = pre_cmds
                .map(|v| {
                    v.iter()
                        .map(|v| v.as_str().ok_or(anyhow::anyhow!("pre_cmd is not a string")))
            ...
            let post_cmds = m
                ...
                .map(|v| {
                    v.as_sequence()
                        .ok_or(anyhow::anyhow!("post_cmds is not a sequence"))
                })
                .transpose()?;
            let post_cmds = post_cmds
                .map(|v| {
                    v.iter()
                        .map(|v| {
                            v.as_str()
                                .ok_or(anyhow::anyhow!("post_cmd is not a string"))
                        })
                        .collect::<anyhow::Result<Vec<&str>>>()
                        .map(|v| v.iter().map(|s| s.to_string()).collect())
                })
